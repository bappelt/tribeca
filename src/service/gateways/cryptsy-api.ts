    ...
    getPositions = (callback) => {
        ...
        this.getCurrencyMap((cryptsyTribecaCurrencyMap:CryptsyTribecaCurrencyMap) => {
            this.makeRequest('/balances', (err, resp, body) => {
                try {
                    var balances = JSON.parse(body).data;
                } catch(e) {
                ...
                for (var currency in Models.Currency) {
                    if (Models.Currency.hasOwnProperty(currency) && !/^\d+$/.test(currency)) {
                        positions[currency] = {
                            'available': balances.available[cryptsyTribecaCurrencyMap.cryptsyIdForCurrency(currency)],
                            'held': balances.held[cryptsyTribecaCurrencyMap.cryptsyIdForCurrency(currency)] || 0
                        }
                    }
                }
                callback(positions);
